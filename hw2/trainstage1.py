...
# train_data = TrainDataset(train=True, transform=config.train_transform)
    train_dataset=loaddataset.TrainDataset(train=True, transform=config.train1_transform)
    train_data=torch.utils.data.DataLoader(train_dataset,batch_size=args.batch_size, shuffle=True, num_workers=WORKER_NUM , drop_last=True)
